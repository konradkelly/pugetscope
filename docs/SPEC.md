...
**Graviton workers, x86 control plane — shipped.** Workers run `t4g.small` (arm64) instead of `t3.small`: identical 2 vCPU / 2 GB, 19% cheaper per hour ($0.0168 vs $0.0208 in us-west-2). Architecture is derived from `instance_type` in the ec2 module rather than declared separately, so there's no second variable to drift out of sync — a mismatch there produces an instance that boots into nothing with no hint in the plan. The real cost of the move is in the image pipeline, not the infrastructure: nothing pins these Deployments to a node, so every image must be a multi-arch manifest list or it fails the first time the scheduler picks the other architecture (`push-ecr.sh` now builds `linux/amd64,linux/arm64` via buildx, with each Dockerfile's build stage on `$BUILDPLATFORM` so only dependency installs are emulated — tsc and Vite output is architecture-independent). `postgis/postgis` turned out to publish no arm64 manifest at all; the schema-init Job only ever used it for a `psql` client, with PostGIS itself running on RDS, so it moved to the official multi-arch `postgres` image. **The control plane stays x86_64** — there's one of it, it holds etcd, and replacing it means downtime, so it belongs to the Phase 3 HA rebuild rather than to a cost change. Migration runbook (add node → join → drain → delete) in `k8s/README.md`. Uncovered while doing this: the module's `most_recent = true` AMI lookups had been silently arming every `terraform apply` to replace both live nodes whenever Canonical published a new image (`ami` is ForceNew), which `ignore_changes = [user_data]` did not cover; `ami` is now ignored too, so a node's image is fixed when it's created and replacements are only ever deliberate.

## 10. Open Questions

- Self-hosting Postgres (K8s pod + EBS PV on the EC2 worker) vs. staying on RDS — see §9. Redis went the self-hosted route; Postgres hasn't, because unlike Redis it holds real data that can't be rebuilt from the next poll cycle, so the calculus isn't just cost. **Worth re-examining, because the number changed:** this was written while RDS was still inside the free tier at ~$1.40/mo, which expired in August 2026 — it now runs ~$13.90/mo (db.t4g.micro $11.67 + 20 GB gp3 $2.26), roughly a quarter of the total AWS bill rather than a rounding error. The argument for staying managed is still the backup/PITR story, not the price; it just has to be worth $14/mo now instead of $1.40.
- **RDS storage has no autoscaling and a finite runway.** `max_allocated_storage = 0` (deliberate, for predictable cost) with 20 GB allocated, ~3.6 GB used as of 2026-08-30 and growing ~2.1 GB/month from `positions` — roughly 8 months before the disk fills, at which point Postgres stops accepting writes and the app goes down. This is an availability question wearing a cost costume: the fix is a retention/downsample policy on `positions` (the rollup tables in `docs/rollup-tables.md` already hold the aggregates worth keeping), not a bigger volume.
...
### 17.3 Airline/route highlights (`fids_flights`) — ✅ built

Goal: "busiest airline," "longest delay," or "notable diversion" for the day, from AeroDataBox data already being fetched.

**The gap (now closed):** `fids_flights` is not historical. `ingestion/src/db/fidsFlights.ts`'s `replaceBoard()` does a full `DELETE` + re-`INSERT` per airport on every refresh, by design (§14) — the table is deliberately *only* ever the current ~3h-ago-to-9h-ahead window, so a flight that's already flown gets purged from the table well before the digest job runs at 10:00 UTC the next day. There was no persisted record of what `fids_flights` looked like for a given past day, so `generateDigest.ts` had nothing to query for "yesterday."

**Fix: `fids_daily_rollup`, a permanent capture table** (`db/init/001_schema.sql`), one row per `(date, airport_icao, direction, call_sign)` where `date` is the flight's LA-local *scheduled* day. `replaceBoard()` upserts a row into it for every flight it writes to `fids_flights`, in the same transaction — not a separate pre-delete sweep as originally sketched above, since capturing the incoming (freshest) data is simpler than reading the outgoing rows back out first. Flights with no `scheduledTime` are skipped (no LA date to bucket them under). Because a flight typically appears across several refreshes before it ages out of the ~12h board window, the same row gets upserted repeatedly with the latest `status`/`revised_time` — so a delay that grows or a status that later flips to "Diverted" is captured too, not just the first sighting. Unlike `traffic_daily_counts`/`overflight_hourly_counts`, this isn't a pre-aggregated count — it stores one row per flight, since "busiest airline" needs a `GROUP BY` and "longest delay"/"notable diversion" need to inspect individual flights, not just a total.

- `loadBusiestAirline()` — `SELECT airline_name, COUNT(*) ... GROUP BY airline_name ORDER BY flights DESC LIMIT 1`.
- `loadLongestDelay()` — largest `revised_time - scheduled_time` gap, filtered to `revised_time > scheduled_time` so an on-time or early flight can never win by default.
- `loadNotableDiversion()` — first captured flight whose `status ILIKE '%divert%'` (AeroDataBox's status vocabulary isn't formally documented, hence the substring match rather than an exact enum).
- `DigestStats` fields: `busiestAirline`, `longestDelay`, `notableDiversion` (all nullable — omitted from the prompt, not sent as zero/unknown, same posture as every other optional fact in this section). All null whenever FIDS enrichment isn't configured (no `AERODATABOX_API_KEY`) or nothing was captured yet.
- No backfill script, unlike `traffic_daily_counts`'s `backfillTrafficRollup.ts`: that script has historical `positions` rows to derive its numbers from, but `fids_flights` was never historical (that's the gap this whole section closes) — there's nothing to backfill `fids_daily_rollup` *from*. It starts empty and accumulates going forward only; early dates simply have no airline/delay/diversion facts.
...
**17.1, 17.2, and 17.3 are built.** Remaining, cheapest first: **17.4 (neighborhood)** needs no new tables and reuses an existing index — a straightforward addition to `loadStats()`, same pattern as 17.1/17.2/17.3. **17.5 (community)** is trivial but not worth adding until `spottings` has real usage.
