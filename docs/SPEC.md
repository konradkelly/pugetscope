...
- Terrain/weather overlays — the terrain half is fully specced (2D hillshade, not weather), see §18.
...
1. **Base style** (mutually exclusive): Bright (current default) · Positron · Liberty · Dark · Topographic.
2. **Terrain relief** (independent boolean), layered on top of whichever base is active — disabled in the UI when Topographic is selected, since OpenTopoMap already bakes in its own relief shading and combining them would just be redundant.
...
- Topographic: OpenTopoMap raster XYZ tiles (`https://{a,b,c}.tile.opentopomap.org/{z}/{x}/{y}.png`), CORS already open. This is a community-run tile server with a fair-use policy discouraging heavy/production traffic — accepted as reasonable for a low-traffic portfolio project, called out explicitly rather than glossed over. Not a style-JSON URL, so it's a minimal inline MapLibre style object built client-side (`version: 8`, one `raster` source carrying the required OSM/OpenTopoMap attribution string, one `raster` layer, no `sprite`/`glyphs`). MapLibre's default `AttributionControl` (already active — the app never sets `attributionControl: false`) rebuilds its displayed text from every active source's `attribution` field automatically, so this alone satisfies the attribution requirement.
